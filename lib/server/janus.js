...
// We want to share the PouchDB constructor with server.js
// so we dont duplicate the options, however some events
// stuff it does screws with us.
// TODO: remove
...
var Janus = function(host, _PouchDB) {
...
Janus.prototype.sendToken = function(email, token, host, cb) {
  ...
  var body = 'Hello!\nAccess your account here: ' +
    this.tokenUrl(token, host, email);

  debug('Sending token to ' + email);
  smtp.send({
    text: body,
...
Janus.prototype.readSession = function(user, host) {
  return new Promise(function(resolve) {
    db.get(activeUserId(user, host), function(err, doc) {
      doc = doc || {};
      delete doc._id;
      delete doc._rev;
      doc.ok = true;
...
Janus.prototype.writeSession = function(user, host, details) {
  return new Promise(function(resolve) {
    db.get(activeUserId(user, host), function(err, doc) {
      ...
      db.put(doc, function(err, written) {
        resolve(doc);
      });
    });
  });
